...
public class ImageLoadAdapter extends BaseAdapter{
    public static final String FROM_GLIDE = "glide";
    public static final String FROM_PICASSO = "picasso";

    private String from;
    private Context mContext;
    private LayoutInflater inflater;
    public ImageLoadAdapter(String form, Context mContext) {
        this.from = form;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (from.equals(FROM_GLIDE)){
            if (position%2==0){
                Glide.with(mContext).load(IMAGES[position]).placeholder(R.mipmap.ic_launcher).error(R.mipmap.default_head_image).centerCrop().crossFade(200).into(viewHolder.iv);
            }else {
                Glide.with(mContext).load(IMAGES[position]).placeholder(R.mipmap.ic_launcher).error(R.mipmap.default_head_image).crossFade(200).into(viewHolder.iv);
            }

        }else if(from.equals(FROM_PICASSO)){
            Picasso.with(mContext).load(IMAGES[position]).placeholder(R.mipmap.ic_launcher).error(R.mipmap.default_head_image).into(viewHolder.iv);
        }
